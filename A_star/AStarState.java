...
public class AStarState
{
    ...
    private Map2D map;
    //These two hashmaps are used to store the collections of "open waypoints"
    //and "closed waypoints", respectively.
    ...
    public Waypoint getMinOpenWaypoint()
    {
        ...
        for (Map.Entry<Location, Waypoint> Entry : this.openWaypoints.entrySet()) {
        	if (Entry.getValue().getTotalCost() < minCost) {
        		minCost = Entry.getValue().getTotalCost();
        		loc = Entry.getKey();
        	}
        }
        return this.openWaypoints.get(loc);
    }
    ...
    {
        if (!this.openWaypoints.containsKey(newWP.loc)) {
        	this.openWaypoints.put(newWP.loc, newWP);
        	return true;
        }
        else if (newWP.getPreviousCost() < this.openWaypoints.get(newWP.loc).getPreviousCost()) {
        	this.openWaypoints.put(newWP.loc, newWP);
        	return true;
        }
        else return false;
    ...
    {
        Waypoint temp = this.openWaypoints.remove(loc);
    	this.closedWaypoints.put(loc, temp);
    }
